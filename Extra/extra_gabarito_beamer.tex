...
\begin{frame}{Questão 1(i): variância de \(u_{i,e}\) (1/2)}
...
\begin{frame}{Questão 1(i): variância de \(u_{i,e}\) (2/2)}
\step{Substituindo \(\operatorname{Cov}(f_i,v_{i,e})=0\) na expressão anterior, obtemos:}
\step{Logo, obtemos:}
...
\begin{frame}{Questão 1(iii): variância da média da firma (1/3)}
...
\begin{frame}{Questão 1(iii): variância da média da firma (2/3)}
...
\step{\highlight{Ponto de transição}: a primeira parcela já é \(\sigma_f^2\); falta desenvolver apenas a segunda parcela.}
\end{frame}

\begin{frame}{Questão 1(iii): variância da média da firma (3/3)}
\step{Retomando a parcela que faltou desenvolver:}
...
\begin{frame}{Questão 2(5): diferença de intercepto entre homens e mulheres}
\begin{block}{Item (5)}
...
\begin{frame}{Questão 3(i): função para \(d=0\) e \(d=1\)}
...
\begin{frame}{Questão 3(ii): ponto de cruzamento}
...
\begin{frame}{Questão 3(iii): igualdade salarial prevista}
...
\begin{frame}{Questão 3(iv): interpretação econômica}
\begin{itemize}
...
\begin{frame}{Questão 4: tabela e leitura geral}
\small
\begin{center}
\begin{tabular}{lcc}
\toprule
...
\begin{frame}{Questão 4(a): comparando colunas (1), (2) e (3) (1/2)}
\begin{itemize}
...
\begin{frame}{Questão 4(a): comparando colunas (1), (2) e (3) (2/2)}
\begin{itemize}
...
\begin{frame}{Questão 4(b): efeito de 10\% nos gastos (1/2)}
...
\begin{frame}{Questão 4(b): efeito de 10\% nos gastos (2/2)}
\step{Retomando o efeito estimado:}
\stepmath{9{,}01\times \log(1{,}10)\approx 0{,}86.}
...
\step{\highlight{Conclusão}: um aumento de 10\% nos gastos está associado a um aumento pequeno, porém estatisticamente significante, na taxa de aprovação em matemática.}
\end{frame}

\begin{frame}{Questão 4(c): termo quadrático (1/2)}
...
\begin{frame}{Questão 4(c): termo quadrático (2/2)}
\step{Retomando o teste do termo quadrático, obtivemos:}
\stepmath{t=\frac{-3{,}45}{2{,}18}\approx -1{,}58.}
\step{Como \(|t|<1{,}96\), a evidência para incluir o termo quadrático é fraca a 5\%.}
\step{\highlight{Conclusão}: a especificação quadrática não recebe suporte forte da evidência estatística ao nível de 5\%.}
\end{frame}

\begin{frame}{Questão 4(d): por que a coluna (2) pode ser melhor que a (3)? (1/2)}
\begin{itemize}
...
\begin{frame}{Questão 4(d): por que a coluna (2) pode ser melhor que a (3)? (2/2)}
\begin{itemize}
    \item Assim, a coluna (2) pode ser mais adequada para medir o efeito total de \textit{lexppp} sobre desempenho em matemática.
\end{itemize}
\end{frame}

\begin{frame}{Questão 4(e): interação entre gastos e pobreza (1/2)}
...
\begin{frame}{Questão 4(e): interação entre gastos e pobreza (2/3)}
\step{Retomando o efeito marginal estimado:}
\stepmath{\frac{\partial math4}{\partial lexppp}=11{,}40-0{,}145\,free.}
\step{Se \(free=60\):}
\stepmath{11{,}40-0{,}145(60)=11{,}40-8{,}70=2{,}70.}
\end{frame}

\begin{frame}{Questão 4(e): interação entre gastos e pobreza (3/3)}
\step{Comparando os dois casos, temos:}
\stepmath{8{,}50 \text{ quando } free=20
\qquad\text{e}\qquad
2{,}70 \text{ quando } free=60.}
